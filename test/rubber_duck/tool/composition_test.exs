defmodule RubberDuck.Tool.CompositionTest do
  ...
  defmodule MockDataFetcher do
    def execute(params, _context) do
      case params do
        %{source: "success"} ->
          {:ok, %{data: "fetched_data", count: 100}}
  ...
  defmodule MockDataSaver do
    def execute(params, _context) do
      case params do
        ...
        _ ->
          {:ok, %{saved: true, location: "/tmp/default"}}
      end
    end
  end
  ...
  describe "sequential/3" do
    test "creates a sequential workflow" do
      workflow =
        Composition.sequential("test_sequential", [
  ...
  describe "parallel/3" do
    test "creates a parallel workflow without merge step" do
      workflow =
        Composition.parallel("test_parallel", [
    ...
    test "handles single step in parallel" do
      workflow =
        Composition.parallel("single_parallel", [
          {:fetch, MockDataFetcher, %{source: "success"}}
        ])
  ...
  describe "conditional/2" do
    test "creates a conditional workflow with both paths" do
      workflow =
        Composition.conditional("test_conditional",
    ...
    test "creates a conditional workflow with only success path" do
      workflow =
        Composition.conditional("test_conditional_success",
          condition: {:check, MockConditionalTool, %{condition: true}},
          success: [
            {:success_action, MockDataTransformer, %{data: "success"}}
          ]
    ...
    test "creates a conditional workflow with only failure path" do
      workflow =
        Composition.conditional("test_conditional_failure",
  ...
  describe "loop/2" do
    ...
    test "creates a loop workflow with aggregator" do
      workflow =
        Composition.loop("test_loop_agg",
          items: ["item1", "item2"],
          steps: [
            {:process, MockDataTransformer, %{action: "transform"}}
          ],
  ...
  describe "execute/3" do
    test "validates workflow execution interface" do
      _workflow =
        Composition.sequential("test_execute", [
          {:fetch, MockDataFetcher, %{source: "success"}}
        ])
    ...
    test "execute_async returns a task" do
      workflow =
        Composition.sequential("test_async", [
          {:fetch, MockDataFetcher, %{source: "success"}}
        ])
  ...
  describe "error handling integration" do
    test "retry step wrapper" do
      retry_step = ErrorHandler.retry_step(MockDataFetcher, %{source: "success"}, max_attempts: 3, initial_delay: 100)
    ...
    test "fallback step wrapper" do
      fallback_step =
        ErrorHandler.fallback_step(MockDataFetcher, %{source: "primary"},
          fallback_tool: MockDataTransformer,
          fallback_params: %{source: "backup"}
        )
    ...
    test "circuit breaker step wrapper" do
      circuit_step =
        ErrorHandler.circuit_breaker_step(MockDataFetcher, %{source: "external"},
          failure_threshold: 5,
