defmodule RubberDuck.Tool.Composition.MonitoringTest do
  ...
  describe "workflow monitoring" do
    test "emits telemetry events for successful workflow" do
      workflow =
        Composition.sequential("test_workflow", [
      ...
      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_start], step1_measurements,
                      step1_metadata}

      assert step1_measurements.count == 1
      assert step1_metadata.step_name == :step1

      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_start], step2_measurements,
                      step2_metadata}
      ...
      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_complete],
                      step1_complete_measurements, step1_complete_metadata}
      ...
      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_complete],
                      step2_complete_measurements, step2_complete_metadata}
      ...
      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_complete],
                      workflow_complete_measurements, workflow_complete_metadata}
    ...
    test "emits telemetry events for failed workflow" do
      ...
      # Verify step events
      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_start], _step1_measurements,
                      _step1_metadata}

      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_complete],
                      _step1_complete_measurements, _step1_complete_metadata}

      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_start], _step2_measurements,
                      _step2_metadata}

      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_error], step2_error_measurements,
                      step2_error_metadata}
      ...
      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_error], workflow_error_measurements,
    ...
    test "parallel workflow monitoring" do
      workflow =
        Composition.parallel("parallel_workflow", [
      ...
      step_complete_events =
        for _ <- 1..3 do
          assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_step_complete], measurements,
                          metadata}
      ...
      assert_receive {:telemetry, [:rubber_duck, :tool, :composition, :workflow_complete], workflow_measurements,
                      workflow_metadata}
  ...
  describe "metrics collection" do
    test "collects workflow metrics" do
      workflow =
        Composition.sequential("metrics_test", [
          {:step1, MockSuccessTool, %{action: "first"}},
          {:step2, MockSuccessTool, %{action: "second"}}
        ])
    ...
    test "tracks failed workflow metrics" do
      workflow =
        Composition.sequential("failure_test", [
          {:step1, MockSuccessTool, %{action: "first"}},
          {:step2, MockFailureTool, %{action: "second"}}
        ])
    ...
    test "provides workflow-specific metrics" do
      workflow =
        Composition.sequential("specific_test", [
          {:step1, MockSuccessTool, %{action: "first"}}
        ])
  ...
  describe "prometheus metrics export" do
    test "exports metrics in prometheus format" do
      workflow =
        Composition.sequential("prometheus_test", [
          {:step1, MockSuccessTool, %{action: "first"}}
        ])
