defmodule RubberDuck.Tool.PerformanceTest do
  ...
  describe "concurrent execution performance" do
    ...
    test "handles mixed load concurrent executions" do
      ...
      params_list =
        Enum.map(load_types, fn load_type ->
          %{load_type: load_type}
        end)
      ...
      # Wait for all results
      results =
        Enum.map(execution_refs, fn {ref, load_type} ->
    ...
    test "maintains performance under high concurrency" do
      ...
      # Average should be low for fast tools
  ...
  describe "throughput testing" do
    ...
    test "handles sustained load" do
      ...
      while System.monotonic_time(:millisecond) < end_time do
        batch_refs =
          Enum.map(1..operations_per_batch, fn i ->
      ...
      # At least 20 ops/second
      assert ops_per_second > 20
      # Should complete a reasonable number
  ...
  describe "scalability testing" do
    test "scales execution with increasing load" do
      ...
      performance_results =
        Enum.map(load_levels, fn concurrency ->
          ...
          # Wait for all results
          results =
            Enum.map(execution_refs, fn ref ->
      ...
      # 10 concurrent should be at least 70% efficiency of 5
      assert ops_10 > ops_5 * 0.7
      # 20 concurrent should be at least 50% efficiency of 5
      assert ops_20 > ops_5 * 0.5
    end
  end
  ...
  describe "error handling performance" do
    test "handles errors efficiently without degrading performance" do
      ...
      results =
        Enum.map(operations, fn {expected_result, params} ->
