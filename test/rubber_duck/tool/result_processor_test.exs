defmodule RubberDuck.Tool.ResultProcessorTest do
  ...
  describe "result processing pipeline" do
    ...
    test "handles transformation options" do
      ...
      # Test sanitization
      assert {:ok, sanitized_result} =
               ResultProcessor.process_result(raw_result, TestTool, @test_context, transform: :sanitize)

      assert sanitized_result.output == "  Hello World!  "

      # Test compression
      assert {:ok, compressed_result} =
               ResultProcessor.process_result(raw_result, TestTool, @test_context, transform: :compress)
    ...
    test "handles validation errors" do
      ...
      assert {:error, :processing_failed, _reason} =
               ResultProcessor.process_result(invalid_result, TestTool, @test_context)
    end
    ...
    test "handles formatting errors" do
      ...
      assert {:error, :formatting_failed, _reason} =
               ResultProcessor.process_result(raw_result, TestTool, @test_context, format: :json)
    end
  end
