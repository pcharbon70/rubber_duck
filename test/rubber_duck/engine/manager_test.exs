defmodule RubberDuck.Engine.ManagerTest do
  ...
  describe "execute/3" do
    ...
    test "executes on specific engine" do
      assert {:ok, "Hello"} = Manager.execute(:manager_test1, %{
        command: "echo",
        text: "Hello"
      })
    end
    ...
    test "respects timeout" do
      assert {:error, :timeout} = Manager.execute(
        :manager_test1,
  ...
  describe "execute_by_capability/3" do
    ...
    test "executes on engine with capability" do
      assert {:ok, "Test"} = Manager.execute_by_capability(:echo, %{
        command: "echo",
        text: "Test"
      })
    end
    ...
    test "supports different selection strategies" do
      # First strategy
      assert {:ok, _} = Manager.execute_by_capability(
        :test,
  ...
  describe "lifecycle management" do
    ...
    test "restart_engine/1" do
      ...
      # Get original pid
      [{_, old_pid}] = Manager.list_engines() 
