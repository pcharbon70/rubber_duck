defmodule RubberDuck.ProcessorTest do
  ...
  describe "Map implementation" do
    ...
    test "process with various options" do
      ...
      test_cases = [
        ...
        # Filter keys
        {[filter_keys: [:user_name, :user_email]], 
         &(map_size(&1) == 2 and Map.has_key?(&1, :user_name))},
        
        # Exclude keys
        {[exclude_keys: [:user_details]], 
         &(not Map.has_key?(&1, :user_details))},
        
        # Stringify keys
        {[stringify_keys: true], 
         &(Map.has_key?(&1, "user_name") and not Map.has_key?(&1, :user_name))},
        
        # Combined options
        {[flatten: true, stringify_keys: true], 
         &(Map.has_key?(&1, "user_details.age") and is_binary(hd(Map.keys(&1))))}
      ]
  ...
  describe "String implementation" do
    ...
    test "metadata extraction" do
      ...
      test_metadata_extraction(string, [
        :type, :encoding, :byte_size, :character_count,
        :line_count, :word_count, :has_unicode
      ])
  ...
  describe "List implementation" do
    ...
    test "process with various options" do
      ...
      test_cases = [
        ...
        # Flattening
        {[flatten: true], &(&1 == list)},  # Already flat
        
        # Combined operations
        {[filter: &(&1 > 2), sort: true, unique: true], 
         &(&1 == [3, 4, 5, 6, 9])}
      ]
    ...
    test "metadata extraction" do
      ...
      test_metadata_extraction(mixed_list, [
        :type, :length, :empty, :element_types,
        :has_nested_lists, :max_depth
      ])
  ...
  describe "performance" do
    ...
    test "processing large datasets" do
      ...
      results = benchmark_protocol_operations(Processor, large_list, [
