defmodule RubberDuck.EngineIntegrationTest do
  ...
  describe "end-to-end engine system" do
    test "loading and using example engines" do
      ...
      # Test echo engine
      assert {:ok, "[ECHO] Hello World"} = Manager.execute(:echo, %{
        text: "Hello World"
      })
      
      # Test reverse engine
      assert {:ok, "dlroW olleH"} = Manager.execute(:reverse, %{
        text: "Hello World"
      })
      
      # Test capability-based execution
      assert {:ok, _result} = Manager.execute_by_capability(:text_processing, %{
        text: "Test"
      })
