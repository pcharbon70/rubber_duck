defmodule RubberDuck.SelfCorrection.LearnerTest do
  ...
  defp seed_learning_data() do
    ...
    for i <- 1..50 do
      ...
      success = case {strategy, content_type} do
        {:syntax, :code} -> rem(i, 10) > 2  # 80% success
        {:semantic, :text} -> rem(i, 10) > 3  # 70% success
        {:logic, _} -> rem(i, 10) > 4        # 60% success
        _ -> rem(i, 2) == 0                  # 50% success
      end
  ...
  describe "get_strategy_rankings/0" do
    ...
    test "syntax strategy ranks high for code" do
      ...
      assert syntax_ranking.success_rate > 0.7  # We seeded it with 80% success
    end
  end
    ...
    test "identifies correlated issues" do
      ...
      for i <- 1..20 do
        History.record_correction(%{
          ...
          strategy: :logic,
          content_type: :code,
          issues_found: [:issue_a, :issue_b],  # Always appear together
    ...
    test "models convergence patterns" do
      ...
      for i <- 1..15 do
        History.record_correction(%{
          ...
          iterations: 3,  # Consistent iterations
          convergence_time: 1.5,  # Consistent time
          metadata: %{}
        })
      end
