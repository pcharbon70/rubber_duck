defmodule RubberDuck.SelfCorrection.EngineTest do
  ...
  describe "correct/1" do
    ...
    test "improves text clarity" do
      request = %{
        content: "This is a very very very long sentence that could definitely be written in a much more clear and concise way without losing any of the important meaning.",
        type: :text,
        context: %{},
        target_score: 0.7
      }
    ...
    test "detects convergence early" do
      ...
      assert {:ok, result} = Engine.correct(request)
      assert result.iterations == 1  # Should stop early
      assert result.success == true
    end
  end
  ...
  describe "strategy integration" do
    test "uses syntax strategy for syntax errors" do
      request = %{
        content: "def broken(x do x end",  # Missing closing paren
        type: :code,
        context: %{language: "elixir"},
        strategies: [:syntax]
      }
