defmodule RubberDuck.Instructions.TemplateProcessorTest do
  ...
  describe "process_template/3" do
    ...
    test "validates template by default" do
      template = String.duplicate("x", 60_000)  # Exceeds max size
  ...
  describe "process_template_with_inheritance/4" do
    test "processes template with includes" do
      ...
      assert {:ok, result} = TemplateProcessor.process_template_with_inheritance(
        template, 
        variables, 
        loader
      )
    ...
    test "processes template with extends" do
      ...
      assert {:ok, result} = TemplateProcessor.process_template_with_inheritance(
        ...
        variables, 
        loader,
        markdown: false  # Disable markdown to prevent HTML escaping
      )
      assert result =~ "Hello World"
    ...
    test "handles loader errors" do
      ...
      assert {:error, _} = TemplateProcessor.process_template_with_inheritance(
        template, 
        %{}, 
        loader
      )
  ...
  describe "validate_template/2" do
    ...
    test "detects EEx syntax errors" do
      template = "Hello <%= name"  # Missing closing tag
  ...
  describe "error handling" do
    ...
    test "handles EEx rendering errors gracefully" do
      ...
      assert {:error, {:render_error, _}} = TemplateProcessor.process_template(
        template, 
        %{}, 
        type: :system
      )
