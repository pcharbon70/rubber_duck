defmodule RubberDuckWeb.ConversationChannelLLMTest do
  ...
  setup do
    socket = socket(RubberDuckWeb.UserSocket, "user_socket:#{@test_user_id}", %{
  ...
  describe "LLM preference management" do
    test "set_llm_preference creates new configuration", %{socket: socket} do
      ...
      {:ok, _profile} = RubberDuck.Memory.create_or_update_profile(%{
        user_id: @test_user_id,
        preferences: %{},
        learned_patterns: %{}
      })
      ...
      # Set LLM preference
      ref = push(socket, "set_llm_preference", %{
        "provider" => "openai",
        "model" => "gpt-4",
        "is_default" => true
      })
