defmodule RubberDuck.Integration.Phase2ActualTest do
  ...
  describe "unified engine interface" do
    ...
    test "engines can be executed through the Manager" do
      ...
      assert {:ok, result} = RubberDuck.Engine.Manager.execute_by_capability(
        :simple_execution,
        "hello world"
      )
  ...
  describe "engine failure handling" do
    ...
    test "engine execution failures are handled" do
      ...
      assert {:error, :execution_failed} = RubberDuck.Engine.Manager.execute_by_capability(
        :fail_on_exec,
        "test"
      )
    end
  ...
  defmodule TestCustomStruct do
    ...
    defimpl RubberDuck.Processor do
      ...
      def validate(struct) do
        if is_struct(struct, RubberDuck.Integration.Phase2ActualTest.TestCustomStruct), do: :ok, else: {:error, :invalid_type}
      end
