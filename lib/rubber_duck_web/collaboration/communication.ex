defmodule RubberDuckWeb.Collaboration.Communication do
  ...
  @reaction_cooldown 500  # milliseconds between reactions
  @pointer_update_rate 50 # milliseconds between pointer updates
  ...
  def handle_cast({:send_quick_message, project_id, user_id, message}, state) do
    if valid_quick_message?(message) do
      quick_msg = %{
        user_id: user_id,
        message: String.slice(message, 0, 100), # Limit length
        timestamp: DateTime.utc_now()
      }
