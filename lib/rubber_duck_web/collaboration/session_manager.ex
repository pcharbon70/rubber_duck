defmodule RubberDuckWeb.Collaboration.SessionManager do
  ...
  def init(_opts) do
    ...
    :timer.send_interval(300_000, :cleanup_ended_sessions) # 5 minutes
  ...
  def handle_call({:create_session, project_id, owner_id, params}, _from, state) do
    ...
    recording_id = if session.is_recording do
  ...
  def handle_info({:record_event, session_id, event}, state) do
    ...
    case get_in(state, [:sessions, session_id]) do
      %{is_recording: true} ->
        state = update_in(state, [:event_buffers, session_id], fn
          nil -> [event]
          buffer -> [event | buffer]
        end)
        {:noreply, state}
  ...
  defp start_collaboration_services(session) do
    ...
    RubberDuckWeb.Collaboration.PresenceTracker.track_user(
      ...
      %{
        ...
        email: "owner@example.com",  # Would get from user data
  ...
  defp save_session_recording(session, state) do
    ...
    %SessionRecording{
      ...
      session_id: session.id,
      events: Enum.reverse(events),  # Restore chronological order
      duration: if(session.ended_at && session.started_at) do
