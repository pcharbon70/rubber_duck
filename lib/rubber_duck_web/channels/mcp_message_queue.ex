defmodule RubberDuckWeb.MCPMessageQueue do
  ...
  def handle_cast({:report_delivery_failure, message_id, error}, state) do
    ...
    case find_message_by_id(state.queue_table, message_id) do
      {:ok, {key, message}} ->
        updated_message = %{message | retry_count: message.retry_count + 1, last_error: error}
        ...
        if updated_message.retry_count >= updated_message.max_retries do
          ...
          Logger.warning(
            "Message #{message_id} moved to dead letter queue after #{updated_message.retry_count} retries"
          )
        else
          ...
          Logger.debug(
            "Message #{message_id} requeued for retry #{updated_message.retry_count}/#{updated_message.max_retries}"
          )
        end
  ...
  defp cleanup_expired_messages(state) do
    ...
    if total_cleaned > 0 do
      Logger.info(
        "Cleaned up #{total_cleaned} expired messages (#{length(expired_messages)} queue, #{length(expired_dlq)} DLQ)"
      )
    end
  end
