defmodule RubberDuck.Engines.Completion.Incremental do
  ...
  def update_session(session, new_prefix, update_type) do
    ...
    %{session | current_completions: ranked_completions, current_prefix: new_prefix, last_updated: DateTime.utc_now()}
  end
  ...
  def session_valid?(session, opts \\ []) do
    # 5 minutes
    max_age = Keyword.get(opts, :max_age_seconds, 300)
    # characters
    _max_deviation = Keyword.get(opts, :max_deviation, 10)
  ...
  def accept_partial(session, accepted_text) do
    ...
    %{session | current_completions: updated_completions, current_prefix: new_prefix, last_updated: DateTime.utc_now()}
  end
  ...
  defp chars_in_order?(text, prefix) do
    ...
    {_, found_all} =
      Enum.reduce(prefix_chars, {text, true}, fn char, {remaining, found} ->
        if found do
