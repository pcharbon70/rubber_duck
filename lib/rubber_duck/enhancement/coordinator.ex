defmodule RubberDuck.Enhancement.Coordinator do
  ...
  def handle_call({:enhance, task, opts}, from, state) do
    ...
    state = put_in(state, [:active_enhancements, enhancement_id], %{
      task: task,
      opts: opts,
      from: from,
  ...
  defp do_enhance(task, opts, config) do
    try do
      # Select techniques
      techniques = if opts[:techniques] do
        opts[:techniques]
      else
  ...
  defp execute_pipeline(pipeline, task, opts) do
    ...
    result = Enum.reduce(pipeline, %{content: task.content, context: task.context}, fn step, acc ->
  ...
  defp apply_technique({:parallel, techniques}, data, opts) do
    ...
    tasks = Enum.map(techniques, fn technique ->
  ...
  defp run_ab_test(task, variants, config) do
    results = Enum.map(variants, fn variant_opts ->
  ...
  defp analyze_ab_results(results) do
    ...
    if Enum.empty?(successful_results) do
      ...
      scored_results = Enum.map(successful_results, fn r ->
