defmodule RubberDuck.Enhancement.PipelineBuilder do
  ...
  @type pipeline_step ::
          technique_config()
          | {:parallel, [pipeline_step()]}
          | {:conditional, condition(), pipeline_step(), pipeline_step()}
  ...
  defp group_parallel_techniques(techniques, config) do
    ...
    |> List.flatten()
    # Simple grouping
    |> Enum.chunk_by(fn _ -> :rand.uniform() > 0.7 end)
  end
    ...
    Enum.reduce(pipeline, {[], seen}, fn
      ...
      {:parallel, steps}, {acc, seen_techniques} ->
        ...
        if Enum.empty?(filtered_steps) do
          ...
          new_seen =
            Enum.reduce(filtered_steps, seen_techniques, fn {tech, _}, s ->
              MapSet.put(s, tech)
            end)
