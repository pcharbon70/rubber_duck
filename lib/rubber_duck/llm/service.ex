defmodule RubberDuck.LLM.Service do
  ...
  defp initialize_rate_limiter(config) do
    case config[:rate_limit] do
      {limit, unit} ->
        ...
        :ex_rated.delete_bucket(bucket_name)
        
        scale_ms = case unit do
          :second -> 1_000
          :minute -> 60_000
          :hour -> 3_600_000
        end
  ...
  defp build_model_mapping(provider_configs) do
    Enum.reduce(provider_configs, %{}, fn config, acc ->
      ...
      Map.merge(acc, Map.new(models, fn model ->
        {model, config.name}
      end))
    end)
  end
  ...
  defp check_rate_limit(%{rate_limiter: rate_limiter}) do
    case :ex_rated.check_rate(rate_limiter.bucket, rate_limiter.scale_ms, rate_limiter.limit) do
      {:ok, _count} -> true
      {:error, _limit} -> false
    end
  end
  ...
  defp perform_health_checks(state) do
    ...
    new_providers = Map.new(state.providers, fn {name, provider_state} ->
    ...
    new_health_monitor = Enum.reduce(new_providers, state.health_monitor, fn {name, provider_state}, monitor ->
  ...
  defp handle_request_completion(request_id, result, state) do
    case Map.get(state.active_requests, request_id) do
      ...
      request ->
        ...
        {status, response_or_error} = case result do
        ...
        new_cost_tracker = case result do
          {:ok, response} ->
        ...
        {new_failures, new_circuit_state} = case result do
          {:ok, _} ->
        ...
        new_provider_state = %{provider_state |
