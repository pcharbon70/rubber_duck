defmodule RubberDuck.LLM.Tokenization do
  ...
  defp count_message_tokens(messages, model, :openai) do
    ...
    base_overhead = case model do
      "gpt-3.5-turbo" -> 3  # Every message follows <|start|>{role/name}\n{content}<|end|>\n
      _ -> 3  # GPT-4 models
    end
    
    message_tokens = Enum.reduce(messages, 0, fn message, acc ->
  ...
  defp count_message_tokens(messages, model, :anthropic) do
    ...
    total_tokens = Enum.reduce(messages, 0, fn message, acc ->
      content = get_message_content(message)
  ...
  defp get_openai_encoding(model) do
    cond do
      ...
      true -> "cl100k_base"  # Default to most common modern encoding
    end
  end
