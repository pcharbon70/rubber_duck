defmodule RubberDuck.Planning.Repository.ChangeImpactAnalyzer do
  ...
  def analyze_impact(repo_analysis, changed_files, opts \\ []) do
    ...
    with {:ok, affected_files} <- get_affected_files(repo_analysis, changed_files),
      ...
      Logger.info(
        "Impact analysis complete: #{length(affected_files.direct)} direct, " <>
  ...
  defp get_affected_files(repo_analysis, changed_files) do
    ...
    {:ok,
     %{
       direct: direct,
       # Remove direct dependencies from transitive
  ...
  defp identify_parallel_groups(repo_analysis, files) do
    ...
    groups =
      ...
      |> Enum.group_by(fn file ->
        ...
        # Group by dependency count within the set
        length(deps_in_set)
      end)
      |> Map.values()
  ...
  defp simulate_change_propagation(repo_analysis, changed_files) do
    ...
    step2_files =
      DependencyGraph.get_dependent_files(repo_analysis.dependencies, step1_files)
      |> Enum.reject(&(&1 in step1_files))

    steps =
      if length(step2_files) > 0 do
        steps ++
          [
            %{
              step: 2,
      ...
      patterns ->
        [
          %{
            ...
            severity: :high,
            description:
              "Changes affect core architectural components: #{Enum.map(patterns, & &1.name) |> Enum.join(", ")}",
