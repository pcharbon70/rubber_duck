defmodule RubberDuck.Planning.Repository.ChangeSequencer do
  ...
  @type conflict_type ::
          :file_modification | :dependency_cycle | :resource_contention | :compilation_order | :test_interference
  ...
  defp sort_by_dependency_level(remaining, dependencies, acc) do
    ...
    independent =
      Enum.filter(remaining, fn change ->
        deps =
          Enum.filter(dependencies, fn {_from, to} -> to == change end)
    ...
    points =
      case validation_strategy do
        ...
        :conservative ->
          ...
          key_phases =
            Enum.filter(phases, & &1.validation_required)
            |> Enum.map(fn phase ->
              %{
                after_phase: phase.phase,
                type: :test_suite,
  ...
  defp create_rollback_plan(phases) do
    checkpoints =
      phases
      # Every 3rd phase or critical phases
      |> Enum.filter(&(&1.validation_required or rem(&1.phase, 3) == 0))
      |> Enum.map(fn phase ->
        %{
          phase: phase.phase,
    ...
    # Rough estimate
    estimated_time = Duration.new!(minute: length(phases) * 2)
  ...
  defp estimate_total_duration(phases, parallel_groups) do
    ...
    parallel_savings =
      ...
      |> Enum.reduce(Duration.new!(second: 0), &Duration.add/2)
      # Assume 60% efficiency in parallel execution
      |> Duration.multiply(0.6)
  ...
  defp detect_file_conflicts(change_requests) do
    ...
    Enum.map(file_changes, fn {file, conflicting_changes} ->
      %{
        ...
        files: [file],
        description:
          "File #{file} modified by multiple changes: #{Enum.map(conflicting_changes, & &1.id) |> Enum.join(", ")}",
        resolution_strategy: %{
    ...
    adjacent_phases =
      ...
      |> Enum.filter(fn [phase1, phase2] ->
        ...
        phase1.can_parallel and phase2.can_parallel and
          length(phase1.files ++ phase2.files) < 10
      end)
