defmodule RubberDuck.Planning.Execution.History do
  ...
  def record_task_start(history, task_id) do
    ...
    %{history |
      timings: Map.put(history.timings, task_id, updated_timing)
    }
  end
  ...
  def increment_retry(history, task_id) do
    ...
    %{history |
      retries: Map.put(history.retries, task_id, current_count + 1)
    }
  end
  ...
  def record_attempt(history, task_id, attempt_data) do
    attempt = Map.merge(attempt_data, %{
      timestamp: DateTime.utc_now(),
      attempt_number: get_attempt_count(history, task_id) + 1
    })
    
    %{history |
      attempts: Map.update(history.attempts, task_id, [attempt], &[attempt | &1])
    }
  end
  ...
  def record_snapshot(history, snapshot_type, data) do
    ...
    %{history |
      snapshots: Map.put(history.snapshots, snapshot_id, snapshot)
    }
  end
  ...
  defp generate_insights(history) do
    ...
    insights = if length(high_retry_tasks) > 0 do
    ...
    insights = if total_attempts > 0 do
    ...
    insights = if length(long_tasks) > 0 do
