defmodule RubberDuck.Planning.Execution.ExecutionState do
  ...
  def initialize(state, tasks) do
    ...
    %{state | all_tasks: task_ids, task_dependencies: dependencies, updated_at: DateTime.utc_now()}
  end
  ...
  def start_task(state, task_id, process_ref) do
    %{
      state
      | current_tasks:
          Map.put(state.current_tasks, task_id, %{
  ...
  def release_resources(state, task_id) do
    %{state | resource_allocations: Map.delete(state.resource_allocations, task_id), updated_at: DateTime.utc_now()}
  end
  ...
  def restore(state, snapshot) do
    %{
      ...
      | completed_tasks: MapSet.new(snapshot.completed_tasks),
        ...
        # Current tasks need to be re-executed
        current_tasks: %{},
