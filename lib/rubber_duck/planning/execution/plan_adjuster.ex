defmodule RubberDuck.Planning.Execution.PlanAdjuster do
  ...
  defp check_failure_threshold(_observation, execution_state) do
    ...
    if failure_rate > 0.3 do  # 30% failure threshold
  ...
  defp check_resource_constraints(%{anomalies: anomalies}) do
    resource_anomaly = Enum.find(anomalies, fn
  ...
  defp check_anomalies(%{anomalies: anomalies}) do
    critical_anomaly = Enum.find(anomalies, fn
      %{severity: :error} -> true
  ...
  defp check_insights_requiring_adjustment(%{insights: insights}) do
    ...
    requires_adjustment = Enum.any?(insights, fn insight ->
  ...
  defp simplify_remaining_tasks(plan, execution_state) do
    ...
    simplified_tasks = Enum.map(remaining_tasks, fn task ->
      simplify_task(task)
    end)
  ...
  defp parallelize_independent_tasks(plan, execution_state) do
    ...
    updated_tasks = Enum.flat_map(parallel_groups, fn group ->
    ...
    |> Enum.map(fn task ->
      case task.metadata[:batch_size] do
        ...
        current_size ->
          ...
          %{task |
            metadata: Map.put(task.metadata, :batch_size, new_size)
          }
      end
    end)
  ...
  defp add_rate_limiting(plan) do
    updated_metadata = Map.merge(plan.metadata || %{}, %{
      rate_limit: %{
        ...
        delay_between_tasks: 1000  # 1 second
      }
    })
  ...
  defp replace_task_in_plan(plan, old_task_id, new_task) do
    ...
    |> Enum.map(fn task ->
      if task.id == old_task_id do
        ...
        # Update dependencies
        %{task |
          dependencies: update_dependencies(task.dependencies, old_task_id, new_task.id)
        }
      end
    end)
  ...
  defp mark_task_as_optional(plan, task_id) do
    ...
    |> Enum.map(fn task ->
      if task.id == task_id do
        %{task |
          metadata: Map.put(task.metadata || %{}, :optional, true)
        }
      else
        task
      end
  ...
  defp create_alternative_plan(original_plan, description) do
    ...
    {:ok, %{original_plan |
      metadata: Map.put(original_plan.metadata || %{}, :alternative_approach, description)
    }}
  end
  ...
  defp general_optimization(plan, execution_state) do
    ...
    optimized_plan = Enum.reduce(optimizations, plan, fn opt_fn, current_plan ->
