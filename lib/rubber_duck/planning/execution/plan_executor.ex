defmodule RubberDuck.Planning.Execution.PlanExecutor do
  ...
  defp execute_plan_async(state) do
    try do
      ...
      state = %{state | 
        execution_state: ExecutionState.initialize(state.execution_state, tasks)
      }
    ...
    case state.status do
      :running ->
        ...
        if Enum.empty?(executable_tasks) and all_tasks_completed?(state, tasks) do
          ...
          state = Enum.reduce(executable_tasks, state, fn task, acc_state ->
  ...
  defp apply_recovery_strategy(state, task_id, _error) do
    ...
    cond do
      ...
      retry_count < max_retries + 2 ->  # Allow some skip attempts
        {:skip, state}
      true ->
        {:abort, state}
    end
