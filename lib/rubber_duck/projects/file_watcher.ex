defmodule RubberDuck.Projects.FileWatcher do
  ...
  def start_link(project_id, opts) do
    GenServer.start_link(__MODULE__, {project_id, opts}, name: {:via, Registry, {@registry, project_id}})
  end
  ...
  def init({project_id, opts}) do
    ...
    case start_file_system(state) do
      ...
      :ignore ->
        ...
        Logger.warning(
          "File system watching not available for project #{project_id} - file watcher will run in degraded mode"
        )
  ...
  defp flush_event_buffer(state) do
    ...
    # Update activity in WatcherManager if available
