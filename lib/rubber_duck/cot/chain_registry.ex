defmodule RubberDuck.CoT.ChainRegistry do
  ...
  def handle_call({:select_chain, content, context}, _from, state) do
    ...
    new_state = %{state | usage_stats: new_stats, selection_history: new_history}
  ...
  def handle_call(:list_chains, _from, state) do
    chains_list =
      Enum.map(state.chains, fn {module, info} ->
        %{
          module: module,
          name: info.name,
  ...
  defp calculate_chain_score(content, context, chain_info) do
    ...
    context_boost =
      case Map.get(context, :preferred_chain_type) do
    ...
    complexity_match =
      if String.length(content) > 500 && chain_info.capabilities.max_steps > 5 do
        0.2
      else
        0.0
      end
