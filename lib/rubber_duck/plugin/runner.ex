defmodule RubberDuck.Plugin.Runner do
  ...
  def handle_call({:execute, input}, _from, state) do
    ...
    result = case Task.yield(task, :infinity) do
      {:ok, {:ok, output, new_plugin_state}} ->
        new_state = %{state | 
          plugin_state: new_plugin_state,
          stats: update_stats(state.stats, :success, start_time)
        }
      ...
      {:ok, {:error, reason, new_plugin_state}} ->
        new_state = %{state | 
          plugin_state: new_plugin_state,
          stats: update_stats(state.stats, :error, start_time)
        }
      ...
      {:exit, reason} ->
        ...
        new_state = %{state | 
          stats: update_stats(state.stats, :crash, start_time)
        }
