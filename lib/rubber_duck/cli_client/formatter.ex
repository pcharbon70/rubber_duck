defmodule RubberDuck.CLIClient.Formatter do
  ...
  defp format_analysis_result(issues) do
    if Enum.empty?(issues) do
      ...
      severity_sections =
        for {severity, severity_issues} <- grouped do
          header = "\n#{String.upcase(to_string(severity))} (#{length(severity_issues)}):"

          issue_lines =
            for issue <- severity_issues do
              [
  ...
  defp format_completion_result(suggestions) do
    if Enum.empty?(suggestions) do
      ...
      suggestion_lines =
        for {suggestion, index} <- Enum.with_index(suggestions, 1) do
          [
  ...
  defp format_health_status(health) do
    ...
    provider_lines =
      for provider <- health.providers do
        health_icon =
          case provider.health do
            "healthy" -> "●"
            "unknown" -> "?"
