defmodule RubberDuck.CLIClient.Commands.LLM do
  ...
  defp handle_subcommand(:disconnect, args, _opts) do
    ...
    case Client.send_command("llm", params) do
      {:ok, result} ->
        message = format_disconnect_result(result, provider)
        {:ok, message}
  ...
  defp format_disconnect_result(%{"message" => message}, _provider) do
    message
  end

  defp format_disconnect_result(%{"error" => error}, provider) do
    "Failed to disconnect from #{provider}: #{error}"
  end

  defp format_disconnect_result(_result, provider) do
