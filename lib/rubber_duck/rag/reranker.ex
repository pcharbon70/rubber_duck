defmodule RubberDuck.RAG.Reranker do
  ...
  def diversity_rerank(results, opts) do
    ...
    {reranked, _} =
      Enum.reduce_while(results, {[], []}, fn result, {selected, embeddings} ->
        if length(selected) >= limit do
          ...
          is_diverse =
            Enum.all?(embeddings, fn selected_embedding ->
