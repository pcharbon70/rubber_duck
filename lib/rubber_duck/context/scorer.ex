defmodule RubberDuck.Context.Scorer do
  ...
  def suggest_improvements(context, query, score_result) do
    ...
    # Check each metric
    suggestions = if score_result.breakdown.relevance < 0.6 do
      ["Include more query-specific information" | suggestions]
    else
      suggestions
    end
    
    suggestions = if score_result.breakdown.diversity < 0.5 do
      ["Add more diverse information sources" | suggestions]
    else
      suggestions
    end
    
    suggestions = if score_result.breakdown.completeness < 0.6 do
      ["Expand context to cover more aspects of the query" | suggestions]
    else
      suggestions
    end
    
    suggestions = if score_result.breakdown.recency < 0.5 do
    ...
    suggestions = if metadata.token_density > 0.8 do
      ["Consider more concise representation" | suggestions]
    else
      suggestions
    end
    
    suggestions = if metadata.source_count < 3 do
  ...
  defp calculate_diversity(context) do
    ...
    count_score = 
      cond do
        ...
        source_count <= 10 -> 1.0
        true -> 1.0 - (source_count - 10) / 20  # Penalty for too many
      end
  ...
  defp calculate_completeness(context, query) do
    ...
    aspect_coverage = 
      if map_size(question_aspects) > 0 do
        ...
        1.0  # No specific aspects to cover
      end
  ...
  defp calculate_recency(context) do
    ...
    case context.strategy do
      :fim -> score  # FIM always uses recent context
      :rag -> score * 0.9  # RAG may use older patterns
      :long_context -> score * 0.95  # Long context includes history
      _ -> score * 0.7
    end
  end
  ...
  defp count_sections(content) do
    ...
    |> String.split(~r/^##\s+/m)
    |> length()
    |> Kernel.-(1)  # Subtract 1 for the first split part
    |> max(0)
  end
end