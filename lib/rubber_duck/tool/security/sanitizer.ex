defmodule RubberDuck.Tool.Security.Sanitizer do
  ...
  @path_traversal_patterns [
    ~r/\.\./,           # Parent directory
    ~r/\.\.%2[fF]/,     # URL encoded parent directory
    ~r/\.\.%5[cC]/,     # Alternative encoding
    ~r/\.\.\\/,         # Windows path traversal
    ~r/%2[eE]\./,       # URL encoded dot
  ...
  def sanitize_params(params) when is_map(params) do
    results = Enum.map(params, fn {name, {type, value}} ->
      result = case type do
        ...
        _ -> {:ok, value} # Unknown types pass through
      end
