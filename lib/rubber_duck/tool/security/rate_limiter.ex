defmodule RubberDuck.Tool.Security.RateLimiter do
  ...
  defp get_or_create_bucket(key, user_id, state) do
    case :ets.lookup(:rate_limiter_buckets, key) do
      ...
      [] ->
        # Get user priority
        priority =
          case :ets.lookup(:user_priorities, user_id) do
            [{^user_id, p}] -> p
            [] -> :normal
          end
        ...
        # Create new bucket
        bucket =
          Map.merge(state.default_config, %{
  ...
  defp refill_bucket(bucket, current_time) do
    ...
    %{bucket | tokens: new_tokens, last_refill: current_time}
  end
  ...
  defp update_circuit_breaker(key, result, state) do
    ...
    updated =
      case {breaker.state, result} do
        {:closed, :failure} ->
          failures = breaker.failures + 1

          if failures >= state.circuit_config.failure_threshold do
            %{breaker | state: :open, failures: failures, last_failure: System.monotonic_time(:millisecond)}
          else
        ...
        {:half_open, :success} ->
          ...
          if successes >= state.circuit_config.success_threshold do
            %{breaker | state: :closed, failures: 0, successes: 0}
          else
            %{breaker | successes: successes}
          end

        {:half_open, :failure} ->
          %{breaker | state: :open, failures: breaker.failures + 1, last_failure: System.monotonic_time(:millisecond)}
