defmodule RubberDuck.Tool.Security.Monitor do
  ...
  @attack_patterns %{
    ...
    rapid_requests: :dynamic,  # Analyzed statistically
  ...
  @anomaly_thresholds %{
    ...
    error_rate: 0.1,
    resource_usage_spike: 2.0  # 2x normal
  }
  ...
  def init(opts) do
    ...
    state = %{
      ...
      baseline_window: Keyword.get(opts, :baseline_window, 3_600_000),  # 1 hour
      alert_cooldown: Keyword.get(opts, :alert_cooldown, 300_000)  # 5 minutes
    }
  ...
  def handle_call({:check_patterns, value, pattern_type}, _from, state) do
    patterns = if pattern_type == :all do
      @attack_patterns
    else
  ...
  defp update_user_behavior(event) do
    ...
    behavior = case :ets.lookup(:user_behavior, user_id) do
      [{^user_id, data}] -> data
      [] -> %{
        request_times: [],
        error_count: 0,
  ...
  defp should_alert?(event, state) do
    ...
    cooldown_passed = is_nil(last_alert) or 
  ...
  defp get_last_alert_time(type, user_id) do
    recent_alerts = :ets.tab2list(:security_alerts)
  ...
  defp analyze_anomalies(state) do
    # Get recent events
    cutoff = DateTime.add(DateTime.utc_now(), -600, :second)  # Last 10 minutes
  ...
  defp update_behavior_baselines(_state) do
    ...
    if length(all_users) > 0 do
      :ets.insert(:security_stats, {:baselines, %{
  ...
  defp get_filtered_alerts(filter) do
    all_alerts = :ets.tab2list(:security_alerts)
    |> Enum.map(fn {_, alert} -> alert end)
  ...
  defp update_stats(event) do
    ...
    stats = case :ets.lookup(:security_stats, {:hourly, hour_key}) do
      [{_, s}] -> s
