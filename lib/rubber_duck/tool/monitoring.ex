defmodule RubberDuck.Tool.Monitoring do
  ...
  defp store_histogram_observation(key, metric) do
    ...
    observations =
      case :ets.lookup(@table_name, hist_key) do
        [{^hist_key, obs}] -> obs
        [] -> []
      end

    # Keep last 1000 observations
    updated = [metric.value | observations] |> Enum.take(1000)
  ...
  defp store_summary_observation(key, metric) do
    ...
    summary =
      case :ets.lookup(@table_name, summary_key) do
  ...
  defp update_metric_aggregates(metric) do
    ...
    aggregate =
      case :ets.lookup(:tool_metrics_aggregates, aggregate_key) do
        [{^aggregate_key, agg}] ->
          agg
    ...
    updated = %{aggregate | labels: MapSet.put(aggregate.labels, metric.labels), last_updated: metric.timestamp}
  ...
  defp calculate_execution_stats(from_timestamp, to_timestamp) do
    ...
    executions =
      :ets.select(:tool_execution_history, [
        {{{:"$1", :"$2", :"$3"}, :"$4"}, [{:andalso, {:>=, :"$1", from_timestamp}, {:"=<", :"$1", to_timestamp}}],
         [:"$4"]}
      ])
  ...
  defp update_execution_success_stats(stats, execution) do
    case execution.event do
      :completed ->
        Map.update!(stats, :successful_executions, &(&1 + 1))
  ...
  defp check_execution_health do
    ...
    # Last 5 minutes
    recent_executions = count_recent_executions(300)
  ...
  defp check_error_rate_health do
    ...
    stats =
      calculate_execution_stats(
        # Last 5 minutes
        System.system_time(:microsecond) - 300_000_000,
  ...
  defp check_resource_health do
    ...
    # MB
    memory_usage = :erlang.memory(:total) / 1_000_000
  ...
  defp calculate_error_stats(time_window_seconds) do
    ...
    errors =
      :ets.select(:tool_execution_history, [
        {{{:"$1", :"$2", :"$3"}, :"$4"}, [{:andalso, {:>, :"$1", since}, {:==, {:map_get, :event, :"$4"}, :failed}}],
         [:"$4"]}
      ])
  ...
  defp schedule_history_cleanup do
    # Every 10 minutes
    Process.send_after(self(), :cleanup_history, @metrics_interval * 10)
  end
end
