defmodule RubberDuck.Tool.Sandbox do
  ...
  def get_default_config(level) do
    ...
    case level do
      ...
      :balanced ->
        %{base_config |
          timeout: 15_000,
          memory_limit: 75_000_000,  # 75MB
          cpu_limit: 5,              # 5 seconds
          network_access: false,
          env_vars: ["PATH", "HOME"]
        }
      ...
      :relaxed ->
        %{base_config |
          timeout: 30_000,
          memory_limit: 150_000_000, # 150MB
          cpu_limit: 15,             # 15 seconds
          network_access: true,
  ...
  defp build_sandbox_config(tool_module, sandbox_level, opts) do
    ...
    tool_security = RubberDuck.Tool.security(tool_module)
    tool_overrides = if tool_security do
      %{
