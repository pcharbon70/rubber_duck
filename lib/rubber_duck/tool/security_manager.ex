defmodule RubberDuck.Tool.SecurityManager do
  ...
  def handle_call({:check_access, tool_module, user_context, params}, _from, state) do
    ...
    required_capabilities =
      case :ets.lookup(:tool_capabilities, tool_module) do
    ...
    result =
      if allowed and restrictions_met do
        :allowed
      else
        :denied
      end
  ...
  def handle_call({:get_policy, identifier}, _from, state) do
    policy =
      case :ets.lookup(:security_policies, identifier) do
        [{^identifier, pol}] -> pol
  ...
  defp get_filtered_audit_logs(filter) do
    all_logs =
      :ets.tab2list(:security_audit_log)
