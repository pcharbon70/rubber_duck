defmodule RubberDuck.Tool.DiscoveryEnhanced do
  ...
  def semantic_search(query, opts \\ []) do
    ...
    results = Enum.map(scored_tools, fn {tool_module, score} ->
  ...
  defp calculate_semantic_score(tool_module, query_terms) do
    ...
    name_boost = if String.contains?(String.downcase(to_string(metadata.name)), Enum.join(query_terms, " ")), do: 10, else: 0
  ...
  defp find_matching_context(tool_module, query_terms) do
    ...
    matching_sentence = Enum.find(sentences, fn sentence ->
  ...
  defp get_recommendation_candidates(patterns, _user_context) do
    ...
    unused_tools = Enum.reject(all_tools, fn tool ->
      Tool.metadata(tool).name in used_tools
    end)
    
    # Add tools that follow common sequences
    sequence_tools = patterns.typical_sequences
  ...
  defp calculate_category_affinity(metadata, patterns) do
    ...
    category_usage = patterns.frequently_used
    |> Enum.map(&Registry.get/1)
    |> Enum.filter(fn
      {:ok, _} -> true
      _ -> false
  ...
  defp calculate_sequence_score(tool_name, patterns) do
    ...
    relevant_sequences = patterns.typical_sequences
  ...
  defp identify_bottlenecks(metrics, expected_duration) do
    ...
    bottlenecks = if metrics.average_duration_ms > expected_duration * 0.8,
      do: ["Execution time approaching timeout" | bottlenecks], else: bottlenecks
    
    bottlenecks = if metrics.error_rate > 5,
      do: ["High error rate" | bottlenecks], else: bottlenecks
    
    bottlenecks = if metrics.max_duration_ms > expected_duration * 1.5,
      do: ["Occasional very slow executions" | bottlenecks], else: bottlenecks
  ...
  defp generate_optimization_suggestions(analysis) do
    ...
    suggestions = case analysis.performance_rating do
    ...
    suggestions = if "High error rate" in analysis.bottlenecks,
      do: ["Add better error handling and validation" | suggestions], else: suggestions
    
    suggestions = if analysis.optimization_potential > 30,
      do: ["Significant optimization potential detected" | suggestions], else: suggestions
    
    suggestions
  end
  
  defp get_executions_for_period(period) do
    filter = case period do
  ...
  defp calculate_performance_trends(grouped) do
    ...
    |> Enum.map(fn {period, execs} ->
      ...
      avg_duration = if length(durations) > 0,
