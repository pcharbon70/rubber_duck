defmodule RubberDuck.Tool.ExternalRegistry do
  ...
  def handle_info(:initial_scan, state) do
    ...
    tools = Registry.list()
    registrations = Enum.reduce(tools, %{}, fn tool_module, acc ->
      case register_tool_internal(tool_module, state) do
        {:ok, registration} ->
  ...
  def handle_info(:periodic_scan, state) do
    ...
    # Register new tools
    new_registrations = Enum.reduce(new_tools, state.registrations, fn tool_module, acc ->
      case register_tool_internal(tool_module, state) do
        {:ok, registration} ->
    ...
    final_registrations = Enum.reduce(removed_tools, new_registrations, fn tool_module, acc ->
      Enum.each(state.external_configs, fn {service, config} ->
        unregister_from_service(tool_module, service, config)
      end)
  ...
  defp register_tool_internal(tool_module, state) do
    ...
    service_results = Enum.map(state.external_configs, fn {service, config} ->
