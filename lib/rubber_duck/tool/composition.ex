defmodule RubberDuck.Tool.Composition do
  ...
  def sequential(_name, steps, _opts \\ []) do
    ...
    {reactor, _} = Enum.reduce(steps, {reactor, nil}, fn {step_name, tool_module, params}, {acc_reactor, prev_step} ->
      ...
      case prev_step do
        ...
        prev_name ->
          ...
          arguments = [%Reactor.Argument{
            name: :input,
            source: %Reactor.Template.Result{name: prev_name}
          }]
          {:ok, new_reactor} = Reactor.Builder.add_step(acc_reactor, step_name, {Step, [tool_module, params]}, arguments)
          {new_reactor, step_name}
      end
    end)
  ...
  def parallel(_name, steps, opts \\ []) do
    ...
    {reactor, step_names} = Enum.reduce(steps, {reactor, []}, fn {step_name, tool_module, params}, {acc_reactor, acc_names} ->
    ...
    case Keyword.get(opts, :merge_step) do
      {merge_name, merge_tool, merge_params} ->
        ...
        arguments = Enum.map(step_names, fn step_name ->
          %Reactor.Argument{
            name: step_name,
  ...
  def conditional(_name, opts) do
    ...
    # Add success path
    {reactor, success_final} = case Keyword.get(opts, :success, []) do
      [] -> {reactor, nil}
      success_steps -> add_sequential_path(reactor, success_steps, condition_name, :success, opts)
    end
    
    # Add failure path
    {reactor, failure_final} = case Keyword.get(opts, :failure, []) do
      [] -> {reactor, nil}
    ...
    case {success_final, failure_final} do
      ...
      {success_step, failure_step} ->
        ...
        {:ok, reactor} = Reactor.Builder.add_step(reactor, :conditional_return, {Step, [RubberDuck.Tool.Composition.ConditionalReturn, %{}]}, [
  ...
  def loop(_name, opts) do
    ...
    {reactor, final_steps} = Enum.with_index(items)
    ...
    case Keyword.get(opts, :aggregator) do
      {agg_name, agg_tool, agg_params} ->
        ...
        arguments = Enum.map(final_steps, fn step_name ->
          %Reactor.Argument{
            name: step_name,
            source: %Reactor.Template.Result{name: step_name}
          }
  ...
  defp add_monitoring_middleware(workflow, opts) do
    ...
    if monitoring_enabled do
      ...
      case Reactor.Builder.add_middleware(workflow, Monitoring) do
        {:ok, monitored_workflow} -> monitored_workflow
        {:error, reason} -> 
  ...
  defp add_sequential_path(builder, steps, condition_step, branch_type, _opts) do
    {builder, prev_step} = Enum.reduce(steps, {builder, condition_step}, fn {step_name, tool_module, params}, {acc_builder, prev_name} ->
      # Create unique step name for this branch
      branch_step_name = :"#{branch_type}_#{step_name}"
      
      # Create arguments based on previous step
      arguments = case prev_name do
        ^condition_step ->
      ...
      {:ok, new_builder} = Reactor.Builder.add_step(acc_builder, branch_step_name, {Step, [tool_module, params]}, arguments)
  ...
  defp create_item_steps(builder, steps, item, index, _opts) do
    {builder, prev_step} = Enum.reduce(steps, {builder, nil}, fn {step_name, tool_module, params}, {acc_builder, prev_name} ->
      ...
      {:ok, new_builder} = Reactor.Builder.add_step(acc_builder, item_step_name, {Step, [tool_module, item_params]}, arguments)
