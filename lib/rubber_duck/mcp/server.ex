defmodule RubberDuck.MCP.Server do
  ...
  def init(opts) do
    ...
    {:ok, transport} = transport_mod.start_link(
      Keyword.merge(transport_opts, [parent: self()])
    )
  ...
  def handle_call(:list_sessions, _from, state) do
    sessions = Enum.map(state.sessions, fn {id, session} ->
      %{
        id: id,
    ...
    if not compatible_version?(client_version, state.protocol_version) do
      ...
      case SessionSupervisor.start_session(state.session_supervisor, %{
        ...
        {:ok, session_pid} ->
          ...
          response = Protocol.build_response(request_id, %{
