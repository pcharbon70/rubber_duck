defmodule RubberDuck.MCP.SessionManager do
    ...
    case validate_token(old_token) do
      {:ok, session} ->
        ...
        if state.config.enable_refresh and time_until_expiry <= state.config.refresh_window do
          ...
          # Update session
          updated_session = %{session | token: new_token, expires_at: new_expires_at, last_activity: now}
  ...
  def handle_call({:list_user_sessions, user_id}, _from, state) do
    session_ids =
      :ets.lookup(:mcp_user_sessions, user_id)
  ...
  def handle_info(:cleanup, state) do
    ...
    expired_count =
      Enum.reduce(expired_sessions, 0, fn {session_id, session}, count ->
