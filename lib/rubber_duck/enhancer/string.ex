defimpl RubberDuck.Enhancer, for: BitString do
  ...
  defp infer_topics(string) do
    ...
    topics =
      if Enum.any?(keywords, &String.match?(&1, ~r/code|program|software|function/i)),
        do: [:programming | topics],
        else: topics

    topics =
      if Enum.any?(keywords, &String.match?(&1, ~r/data|database|query|table/i)), do: [:data | topics], else: topics

    topics =
      if Enum.any?(keywords, &String.match?(&1, ~r/user|customer|client|person/i)), do: [:people | topics], else: topics

    topics =
      if Enum.any?(keywords, &String.match?(&1, ~r/system|server|network|cloud/i)),
        do: [:infrastructure | topics],
        else: topics
  ...
  defp extract_dates(string) do
    patterns = [
      ...
      ~r/\d{1,2}-\d{1,2}-\d{2,4}/,
      # Month name
      ~r/\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\s+\d{1,2},?\s+\d{4}/i
    ]
