defmodule RubberDuck.SelfCorrection.History do
  ...
  def handle_cast({:record_correction, data}, state) do
    ...
    new_state = if updated_state.entry_count > @max_history_size do
  ...
  def handle_call({:analyze_patterns, opts}, _from, state) do
    ...
    analysis = case Map.get(state.pattern_cache, cache_key) do
      nil ->
        # Perform analysis
  ...
  defp update_metrics(state, entry) do
    ...
    {success_counts, failure_counts} = if entry.success do
      {
  ...
  defp calculate_success_metrics(state) do
    ...
    overall_success_rate = if total_attempts > 0 do
      total_successes / total_attempts
    else
      0.0
    end
    
    strategy_metrics = Map.merge(state.success_counts, state.failure_counts, fn _k, s, f ->
      total = s + f
      %{
        successes: s,
        failures: f,
