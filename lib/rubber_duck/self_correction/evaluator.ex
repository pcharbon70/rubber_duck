defmodule RubberDuck.SelfCorrection.Evaluator do
  ...
  defp evaluate_syntax_quality(content) do
    ...
    balanced = Enum.all?(delimiter_pairs, fn {open, close} ->
  ...
  defp evaluate_semantic_quality(content) do
    ...
    good_naming = if Regex.match?(~r/def|function|class/, content) do
    ...
    clarity_score = cond do
      avg_word_length < 3 -> 0.5  # Too simple
      avg_word_length > 12 -> 0.6  # Too complex
      true -> 0.8
    end
  ...
  defp evaluate_logical_quality(content) do
    ...
    if Regex.match?(~r/if|while|for|case/, content) do
      ...
      has_infinite_loop = Regex.match?(~r/while\s*\(\s*true\s*\)|loop\s*do/, content) &&
  ...
  defp evaluate_clarity(content) do
    ...
    avg_sentence_length = if length(sentences) > 0 do
      total_words = Enum.sum(Enum.map(sentences, fn s ->
    ...
    clarity_score = cond do
      avg_sentence_length < 5 -> 0.6   # Too terse
      avg_sentence_length > 30 -> 0.5  # Too complex
      avg_sentence_length > 20 -> 0.7  # Getting long
      true -> 0.9                      # Good range
    end
  ...
  defp evaluate_coherence(content) do
    ...
    if Regex.match?(~r/^\s{2,}/, content) do
      ...
      if length(words) > 10 do
        ...
        cond do
          repetition_ratio < 0.1 -> 0.6  # Too scattered
          repetition_ratio > 0.5 -> 0.7  # Too repetitive
          true -> 0.85                   # Good balance
        end
      else
        0.7  # Default for short content
      end
    end
  end
  ...
  defp enhance_with_text_evaluation(eval, content, _context) do
    ...
    flesch_score = if length(sentences) > 0 && length(words) > 0 do
  ...
  defp enhance_with_mixed_evaluation(eval, content, context) do
    ...
    code_weight = if total_length > 0 do
  ...
  defp calculate_overall_score(evaluation) do
    ...
    all_dimensions = Map.merge(
      %{readability: 0.0},  # Defaults
      dimensions
    )
    
    weighted_sum = Enum.reduce(weights, 0.0, fn {dim, weight}, acc ->
  ...
  defp estimate_avg_syllables(words) do
    ...
    total_syllables = Enum.sum(Enum.map(words, fn word ->
