defmodule RubberDuck.SelfCorrection.Strategies.Syntax do
  ...
  @impl true
  def priority(), do: 100  # High priority - syntax must be correct first
  ...
  defp detect_elixir_syntax_issues(content) do
    ...
    case Code.string_to_quoted(content) do
      ...
      {:error, error} ->
        [issue(:syntax_error, :error, "Syntax error: #{inspect(error)}", 
          %{}, %{parser_error: error}) | issues]
    end
  end
  ...
  defp check_unmatched_delimiters(content, [open_delim, close_delim]) do
    ...
    {_, issues} = Enum.reduce(lines, {0, []}, fn line, {depth, issues} ->
      ...
      cond do
        new_depth < 0 ->
          issue = issue(:unmatched_delimiter, :error, 
            "Unmatched '#{close_delim}' without corresponding '#{open_delim}'",
    ...
    if elem({0, issues}, 0) > 0 do
      [issue(:unmatched_delimiter, :error, 
        "Unclosed '#{open_delim}' - missing '#{close_delim}'", %{}) | issues]
    else
      issues
    end
  end
  ...
  defp check_unmatched_quotes(content) do
    ...
    if rem(single_quotes, 2) != 0 do
      issues = [issue(:unmatched_quote, :error, "Unmatched single quote", %{}) | issues]
    end
    
    if rem(double_quotes, 2) != 0 do
      issues = [issue(:unmatched_quote, :error, "Unmatched double quote", %{}) | issues]
    end
  ...
  defp check_missing_commas(content) do
    ...
    if Regex.match?(pattern, content) do
      [issue(:missing_comma, :warning, 
        "Possible missing comma between map/list elements", %{})]
    else
      []
    end
  end
  ...
  defp check_invalid_atoms(content) do
    ...
    if Regex.match?(pattern, content) do
      [issue(:invalid_atom, :error, 
        "Invalid atom syntax - atoms cannot start with numbers", %{})]
    else
      []
    end
  end
  ...
  defp check_pipe_operator_usage(content) do
    ...
    |> Enum.flat_map(fn {line, line_num} ->
      if String.ends_with?(String.trim(line), "|>") do
        [issue(:incomplete_pipe, :error, 
          "Pipe operator at end of line without continuation", 
          %{line: line_num})]
      else
        []
      end
  ...
  defp check_semicolon_consistency(content) do
    ...
    if with_semicolon > 0 && without_semicolon > 0 && without_semicolon > with_semicolon * 0.2 do
      [issue(:inconsistent_semicolons, :warning, 
  ...
  defp check_arrow_function_syntax(content) do
    ...
    if Regex.match?(~r/=>\s*\n\s*{/, content) && !Regex.match?(~r/\)\s*=>\s*\n\s*{/, content) do
      [issue(:invalid_arrow_function, :error, 
  ...
  defp generate_corrections(content, issues, type, context) do
    language = context[:language] || detect_language(content)
  ...
  defp generate_correction_for_issue(content, %{type: :unmatched_delimiter} = issue, _language) do
    # Try to fix unmatched delimiters
    delimiter = extract_delimiter_from_description(issue.description)
    
    changes = case delimiter do
  ...
  defp detect_language(content) do
    cond do
      ...
      String.contains?(content, ["def ", "import ", "class "]) && 
        String.contains?(content, [":", "    "]) -> "python"
  ...
  defp apply_correction_changes(content, changes) do
    try do
      corrected = Enum.reduce(changes, content, fn change, acc ->
