defmodule RubberDuck.SelfCorrection.Strategies.Semantic do
  ...
  @impl true
  def priority(), do: 80  # Lower than syntax but still important
  ...
  defp detect_code_semantic_issues(content, context) do
    ...
    issues = issues ++ check_variable_naming(content, language)
  ...
  defp detect_text_semantic_issues(content, _context) do
    ...
    issues = issues ++ check_text_clarity(content)
  ...
  defp detect_evaluation_based_issues(_content, evaluation) do
    ...
    if Map.get(evaluation, :clarity_score, 1.0) < 0.7 do
      issues = [issue(:low_clarity, :warning, 
        "Content clarity is below threshold", %{}, 
        %{clarity_score: evaluation.clarity_score}) | issues]
    end
    
    # Check coherence
    if Map.get(evaluation, :coherence_score, 1.0) < 0.7 do
      issues = [issue(:low_coherence, :warning,
        "Content coherence needs improvement", %{},
        %{coherence_score: evaluation.coherence_score}) | issues]
    end
  ...
  defp check_variable_naming(content, "elixir") do
    ...
    if length(matches) > 2 do
      issues = [issue(:poor_variable_naming, :warning,
        "Multiple single-letter variables detected", %{}) | issues]
    end
    
    # Non-snake_case variables
    camelcase_pattern = ~r/\b([a-z]+[A-Z][a-zA-Z]*)\s*=/
    if Regex.match?(camelcase_pattern, content) do
      issues = [issue(:naming_convention, :warning,
        "Variables should use snake_case in Elixir", %{}) | issues]
    end
  ...
  defp check_variable_naming(content, _language) do
    ...
    issues = Enum.flat_map(vague_names, fn name ->
      pattern = Regex.compile!("\\b#{name}\\d*\\s*=")
      if Regex.match?(pattern, content) do
        [issue(:vague_variable_name, :info,
          "Variable name '#{name}' is too generic", %{})]
      else
        []
      end
    end)
  ...
  defp check_function_clarity(content, "elixir") do
    ...
    Enum.flat_map(functions, fn [func_name] ->
      ...
      if !Regex.match?(doc_pattern, content) && String.length(func_name) > 5 do
        [issue(:missing_documentation, :info,
          "Function '#{func_name}' lacks documentation", %{})]
      else
        []
      end
    end)
  ...
  defp check_documentation_quality(content, _language) do
    ...
    if length(todos) > 3 do
      issues = [issue(:excessive_todos, :warning,
        "Multiple TODO/FIXME comments found", %{},
        %{count: length(todos)}) | issues]
    end
    
    # Check for commented out code
    commented_code_lines = content
    |> String.split("\n")
    |> Enum.count(fn line ->
    ...
    if commented_code_lines > 5 do
      issues = [issue(:commented_code, :warning,
        "Excessive commented-out code detected", %{},
        %{lines: commented_code_lines}) | issues]
    end
  ...
  defp check_style_consistency(content, "elixir") do
    ...
    if nested_calls > 3 && pipelines > 0 do
      issues = [issue(:inconsistent_style, :info,
        "Inconsistent use of pipelines vs nested function calls", %{}) | issues]
    end
  ...
  defp check_dead_code(content, _language) do
    ...
    unreachable_patterns = [
      ~r/return.*\n\s*\w+/,  # Code after return
      ~r/raise.*\n\s*\w+/,   # Code after raise
      ~r/throw.*\n\s*\w+/    # Code after throw
    ]
    
    Enum.flat_map(unreachable_patterns, fn pattern ->
      if Regex.match?(pattern, content) do
        [issue(:unreachable_code, :warning,
  ...
  defp check_text_clarity(content) do
    ...
    long_sentences = Enum.filter(sentences, fn s -> 
      word_count = length(String.split(s, ~r/\s+/))
      word_count > 40
    end)
    
    if length(long_sentences) > 0 do
      issues = [issue(:long_sentences, :info,
        "#{length(long_sentences)} sentences are too long (>40 words)", %{}) | issues]
    end
    ...
    if length(passive_matches) > 5 do
      issues = [issue(:passive_voice, :info,
        "Excessive use of passive voice", %{},
        %{count: length(passive_matches)}) | issues]
    end
  ...
  defp check_redundancy(content) do
    ...
    if map_size(trigrams) > 0 do
      ...
      [issue(:redundancy, :info,
        "Repeated phrases detected: #{repeated}", %{})]
    else
      []
    end
  end
  ...
  defp check_ambiguous_references(content) do
    ...
    issues = Enum.flat_map(pronouns, fn pronoun ->
      pattern = Regex.compile!("\\b#{pronoun}\\b", "i")
      matches = Regex.scan(pattern, content)
      
      if length(matches) > 5 do
        [issue(:ambiguous_reference, :info,
          "Frequent use of ambiguous pronoun '#{pronoun}'", %{},
  ...
  defp check_terminology_consistency(content) do
    ...
    Enum.flat_map(term_variations, fn terms ->
      used_terms = Enum.filter(terms, fn term ->
        Regex.match?(Regex.compile!("\\b#{term}\\b", "i"), content)
      end)
      
      if length(used_terms) > 1 do
        [issue(:inconsistent_terminology, :info,
          "Inconsistent use of terms: #{Enum.join(used_terms, ", ")}", %{})]
      else
        []
      end
    end)
  ...
  defp generate_semantic_corrections(content, issues, type, context) do
    ...
    high_priority_issues = Enum.filter(issues, fn issue ->
  ...
  defp suggest_llm_corrections(content, issues, type, context) do
    # Group similar issues
    grouped_issues = Enum.group_by(issues, & &1.type)
    
    Enum.map(grouped_issues, fn {issue_type, issue_list} ->
      case issue_type do
        :poor_variable_naming ->
          correction(:improve_naming,
        ...
        _ ->
          correction(:general_improvement,
            "Improve #{issue_type}",
            [],
            0.5,
            :low)
      ...
      Enum.reduce(lines, {[], [], :text, [], 0}, fn line, {code, text, mode, current, line_num} ->
        cond do
          String.starts_with?(line, "```") && mode == :text ->
            # Start of code block
            {code, [{Enum.join(current, "\n"), %{start_line: line_num - length(current)}} | text], 
             :code, [], line_num + 1}
          
          String.starts_with?(line, "```") && mode == :code ->
            # End of code block
            {[{Enum.join(current, "\n"), %{start_line: line_num - length(current)}} | code], 
             text, :text, [], line_num + 1}
