defmodule RubberDuck.Instructions.PerformanceBenchmark do
  ...
  def benchmark_security_pipeline(templates, iterations) do
    results = Enum.map(templates, fn {name, template, variables} ->
  ...
  def measure_memory_usage(template, variables, iterations \\ 100) do
    ...
    {time_with_security, memory_with_security} = measure_with_memory(fn ->
    ...
    {time_without_security, memory_without_security} = measure_with_memory(fn ->
      Enum.each(1..iterations, fn _ ->
  ...
  def stress_test(opts \\ []) do
    ...
    tasks = Enum.map(1..concurrent_processes, fn process_id ->
      Task.async(fn ->
        process_results = Enum.map(1..requests_per_process, fn request_id ->
  ...
  defp default_templates do
    [
      ...
      {"conditional", "{% if show_greeting %}Hello {{ name }}{% endif %}", %{"show_greeting" => true, "name" => "User"}},
      {"loop", "{% for item in items %}{{ item }}{% endfor %}", %{"items" => ["a", "b", "c"]}},
      {"complex", "{% if user.active %}Welcome {{ user.name | capitalize }}! You have {{ user.messages | size }} messages.{% endif %}", %{"user" => %{"active" => true, "name" => "john", "messages" => [1, 2, 3]}}}
    ]
  end
end