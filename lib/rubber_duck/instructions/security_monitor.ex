defmodule RubberDuck.Instructions.SecurityMonitor do
  ...
  def init(opts) do
    ...
    state = %{
      config: %{
        injection_threshold:
          Keyword.get(opts, :injection_threshold, Map.get(alert_thresholds, :injection_threshold, 3)),
        anomaly_sensitivity:
          Keyword.get(opts, :anomaly_sensitivity, Map.get(alert_thresholds, :anomaly_sensitivity, :medium)),
  ...
  defp check_immediate_threats(user_id, event_type, state) do
    ...
    if event_type == :injection_attempt do
      # 5 minutes
      recent_attempts = count_user_events(user_id, :injection_attempt, 300)
  ...
  defp detect_anomalies(user_id, sensitivity) do
    ...
    # Last 5 minutes
    recent_events = get_user_recent_events(user_id, 300)

    anomalies =
      Enum.filter(recent_events, fn {_timestamp, _user, event_type, metadata} ->
        case :ets.lookup(:anomaly_baselines, {user_id, event_type}) do
  ...
  defp count_user_events(user_id, event_type, window) do
    ...
    :ets.select_count(:security_events, [
      {
        {:"$1", :"$2", :"$3", :"$4"},
        [{:andalso, {:andalso, {:==, :"$2", user_id}, {:==, :"$3", event_type}}, {:>, :"$1", cutoff}}],
        [true]
      }
    ])
  end
  ...
  defp block_user(user_id) do
    case :ets.lookup(:user_profiles, user_id) do
      [{^user_id, profile}] ->
        # Max threat score
        updated_profile = %{profile | threat_score: 1000}
        :ets.insert(:user_profiles, {user_id, updated_profile})
