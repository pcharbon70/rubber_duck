defmodule RubberDuck.Instructions.SecurityAudit do
  ...
  actions do
    ...
    read :recent_events do
      ...
      filter expr(
        inserted_at > ago(^arg(:minutes), :minute)
      )
    ...
    read :find_events do
      ...
      filter expr(
        if not is_nil(^arg(:user_id)), do: user_id == ^arg(:user_id), else: true
      )
      
      filter expr(
        if not is_nil(^arg(:event_type)), do: event_type == ^arg(:event_type), else: true
      )
      
      filter expr(
        if not is_nil(^arg(:severity)), do: severity == ^arg(:severity), else: true
      )
      
      filter expr(
        if not is_nil(^arg(:start_date)), do: inserted_at >= ^arg(:start_date), else: true
      )
      
      filter expr(
        if not is_nil(^arg(:end_date)), do: inserted_at <= ^arg(:end_date), else: true
      )
  ...
  def cleanup_old_logs(days_to_keep \\ 30) do
    ...
    case Ash.Query.for_read(__MODULE__, :read) |> Ash.read() do
      {:ok, all_records} ->
        old_records = Enum.filter(all_records, fn record ->
          DateTime.compare(record.inserted_at, cutoff_date) == :lt
        end)
        
        # Delete old records
        deleted_count = Enum.reduce(old_records, 0, fn record, count ->
