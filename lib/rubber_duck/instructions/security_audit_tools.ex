defmodule RubberDuck.Instructions.SecurityAuditTools do
  ...
  def generate_security_report(opts \\ []) do
    # hours
    timeframe = Keyword.get(opts, :timeframe, 24)
  ...
  def analyze_user_security(user_id, opts \\ []) do
    # 7 days
    timeframe = Keyword.get(opts, :timeframe, 168)
  ...
  defp generate_summary(events) do
    ...
    event_counts =
      Enum.group_by(events, & &1.event_type)
  ...
  defp generate_recommendations(events) do
    ...
    recommendations =
      if violation_rate > 0.1 do
        [
          "Consider stricter security policies - high violation rate: #{Float.round(violation_rate * 100, 1)}%"
          | recommendations
        ]
      else
        recommendations
    ...
    recommendations =
      if length(rate_limit_events) > 50 do
    ...
    recommendations =
      if length(anomaly_events) > 10 do
