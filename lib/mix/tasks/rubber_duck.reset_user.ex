defmodule Mix.Tasks.RubberDuck.ResetUser do
  ...
  defp show_usage do
    Mix.shell().info("Usage: mix rubber_duck.reset_user --current-username USERNAME [--new-username NEW_USERNAME] [--password NEW_PASSWORD]")
    Mix.shell().info("")
  ...
  defp reset_user(current_username, new_username, new_password) do
    ...
    case user do
      ...
      {:ok, user} ->
        ...
        user = case List.keyfind(results, :username, 0) do
          {:username, :ok, updated_user} -> updated_user
          _ -> user
        end
        
        # Update password if provided
        results = if new_password do
  ...
  defp update_username(user, new_username) do
    ...
    existing = RubberDuck.Accounts.User
