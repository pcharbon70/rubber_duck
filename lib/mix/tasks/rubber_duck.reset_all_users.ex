  ...
  def run(args) do
    ...
    cond do
      is_nil(username) ->
        ...
        Mix.shell().info(
          "Usage: mix rubber_duck.reset_all_users --username USERNAME --password PASSWORD [--email EMAIL]"
        )
    ...
    try do
      ...
      case create_admin_user(username, password, email) do
        ...
        {:error, error} ->
          ...
          Mix.shell().error(
            "⚠️  WARNING: Database is now empty! Run this command again or use mix rubber_duck.create_admin to create a user."
          )
      end
    rescue
      e ->
